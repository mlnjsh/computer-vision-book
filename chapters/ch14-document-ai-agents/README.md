# Chapter 14: Document AI and Visual Agents

> **Status:** 📋 Not started  ·  **Target words:** ~3,000  ·  **Hands-on:** Invoice-extraction-to-database pipeline + a simple browser-automation agent driven by vision

> 📘 **Detailed lesson plan:** see [`LESSON.md`](LESSON.md) for the ordered TOC, definitions, theory, notebook cells, and exercises.

## Outline (4-5 lines)
OCR (PaddleOCR, Tesseract, Donut), layout analysis (LayoutLMv3, Doctr), table extraction, structured-output VLMs, and the new visual agent loop: screenshot → reason → click. Walks through Claude Computer Use and OpenAI's vision agent pattern.

## Learning Outcomes
*(to be expanded during drafting — list 3-4 concrete things the reader can do after this chapter)*
- [ ]
- [ ]
- [ ]

## Topics Covered
*(replace with bullet list of the section headings you'll write)*
- [ ]

## Hands-On Project
**Invoice-extraction-to-database pipeline + a simple browser-automation agent driven by vision**

- Notebook: [`../../notebooks/ch14_document-ai-agents.ipynb`](../../notebooks/)
- Case study: [`../../case-studies/ch14/`](../../case-studies/)
- Figures: [`../../figures/ch14/`](../../figures/)

## Datasets (Kaggle)
- `urbikn/sroie-datasetv2` — `kaggle datasets download -d urbikn/sroie-datasetv2`
- `robikscube/textocr-text-extraction-from-images-dataset` — `kaggle datasets download -d robikscube/textocr-text-extraction-from-images-dataset`

## Reference GitHub Repos
- https://github.com/PaddlePaddle/PaddleOCR
- https://github.com/microsoft/unilm
- https://github.com/clovaai/donut
- https://github.com/anthropics/anthropic-quickstarts

## Papers / Reading
*(populated during research phase — add 5-10 canonical papers + 2-3 blog posts)*
- [ ]

## Draft Checklist
- [ ] Outline finalized
- [ ] Notebook runs end-to-end on Colab T4
- [ ] All figures generated and saved to `figures/ch14/`
- [ ] Prose draft (~3,000 words)
- [ ] Code blocks tested and pinned
- [ ] References + further reading section
- [ ] Self-edit pass (clarity, voice consistency)
- [ ] Technical reviewer feedback incorporated
